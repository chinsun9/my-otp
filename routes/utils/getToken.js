...
function getToken(K, isMinus1 = false) {
  ...
  const nHex = (`000000000000000${N.toString(16)}`).substr(-16); // 16자리가 되도록 앞에 0 패딩추가
  ...
  // OTP 생성
  const offset = parseInt(Number(`0x${hmacHash[hmacHash.length - 1]}`), 10);
  ...
  toeknHex += (
    `00${(Number(`0x${tokenHex4bytes.substring(0, 2)}`) & 0x7f).toString(16)}`
  ).substr(-2);

  for (let index = 2; index < tokenHex4bytes.length; index += 2) {
    const element = tokenHex4bytes.substring(index, index + 2);
    toeknHex += (`00${(Number(`0x${element}`) & 0xff).toString(16)}`).substr(
      -2,
    );
  }
